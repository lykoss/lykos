...
async def game_stats(wrapper: MessageDispatcher, message: str):
    ...
    if wrapper.public:
        ...
        if wrapper.game_state and wrapper.game_state.in_game and wrapper.target is channels.Main:
            await wrapper.pm(messages["stats_wait_for_game_end"])
            return
    ...
    if msg and not msg[0].isdigit():
        ...
        if gamemode != "*":
            ...
            if matches:
                ...
                await wrapper.pm(messages["invalid_mode"].format(msg[0]))
                return
            else:
    ...
    if not gamesize:
        await wrapper.send(db.get_game_totals(gamemode))
    else:
        # Attempt to find game stats for the given game size
        await wrapper.send(db.get_game_stats(gamemode, gamesize))

@command("playerstats", pm=True)
async def player_stats(wrapper: MessageDispatcher, message: str):
    ...
    if wrapper.public:
        ...
        if wrapper.game_state and wrapper.game_state.in_game and wrapper.target is channels.Main:
            await wrapper.pm(messages["no_command_in_channel"])
            return
    ...
    if account is None:
        ...
        if user is wrapper.source:
            key = "not_logged_in"
        await wrapper.pm(messages[key].format(params[0]))
        return
    ...
    if len(params) < 2:
        ...
        if len(matches) == 0:
            ...
            await wrapper.send(messages["ambiguous_role"].format([m.singular for m in matches]))
            return
...
async def my_stats(wrapper: MessageDispatcher, message: str):
    """Get your own stats."""
    msg = message.split()
    await player_stats.func(wrapper, " ".join([wrapper.source.nick] + msg))

@command("rolestats", pm=True)
async def role_stats(wrapper: MessageDispatcher, message: str):
    ...
    if params[-1] == "*" and not roles:
        ...
        if not roles:
            if len(roles) > 0:
                ...
                await wrapper.pm(messages["no_such_role"].format(role))
            return

    if roles:
        await wrapper.pm(db.get_role_stats(roles.get().key))
        return
    ...
    await wrapper.pm(db.get_role_stats(roles.get().key, gamemode))
