...
def setup_variables(rolename, *, knows_totem):
    ...
    async def on_revealroles(evt: Event, var: GameState, user, role):
    ...
    async def on_transition_day_begin2(evt: Event, var: GameState):
        LASTGIVEN.clear()
    ...
    async def add_shaman(evt: Event, chances: dict[str, dict[str, int]]):
...
async def get_totem_target(var: GameState, wrapper: MessageDispatcher, message, lastgiven, totems) -> tuple[Optional[str], Optional[users.User]]:
...
async def give_totem(var: GameState, wrapper: MessageDispatcher, target: User, totem: str, *, key, role) -> Optional[tuple[users.User, users.User]]:
...
async def on_see(evt: Event, var: GameState, seer, target):
...
async def on_day_vote(evt: Event, var: GameState, votee, voters):
...
async def set_all_totems(evt: Event, chances: dict[str, dict[str, int]]):
    chances.update({
        "death"         : {},
        "protection"    : {},
        "silence"       : {},
