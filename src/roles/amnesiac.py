...
async def update_amnesiac(evt: Event, var: GameState, player: User, old_role: Optional[str]):
...
async def on_revealing_totem(evt: Event, var: GameState, user: User, role: str):
...
async def on_update_stats(evt: Event, var: GameState, player: User, mainrole: str, revealrole: str, allroles: set[str]):
