...
@hook("whoreply")
async def who_reply(cli, bot_server, bot_nick, chan, ident, host, server, nick, status, hopcount_gecos, *, tags):
...
@hook("whoisuser")
async def on_whois_user(cli, bot_server, bot_nick, nick, ident, host, sep, realname, *, tags):
...
@hook("whoisaccount")
async def on_whois_account(cli, bot_server, bot_nick, nick, account, logged, *, tags):
...
@hook("whoischannels")
async def on_whois_channels(cli, bot_server, bot_nick, nick, chans, *, tags):
...
@hook("away")
async def on_away(cli, bot_server, bot_nick, nick, message, *, tags):
...
@hook("endofwhois")
async def on_whois_end(cli, bot_server, bot_nick, nick, message, *, tags):
...
@hook("loggedin")
async def on_loggedin(cli, server, nick, rawnick, account, message, *, tags):
...
@hook("ping")
async def on_ping(cli, prefix, server, *, tags):
...
@hook("featurelist")
async def get_features(cli, server, nick, *features, tags):
...
@hook("channelmodeis")
async def current_modes(cli, server, bot_nick, chan, mode, *targets, tags):
...
@hook("channelcreate")
async def chan_created(cli, server, bot_nick, chan, timestamp, *, tags):
...
@hook("mode")
async def mode_change(cli, rawnick, chan, mode, *targets, tags):
...
@hook("banlist")
async def check_banlist(cli, server, bot_nick, chan, target, setter, timestamp, *, tags):
...
@hook("quit")
async def on_quit(cli, rawnick, reason, *, tags):
