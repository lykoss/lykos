...
async def on_del_player(evt: Event, var: GameState, player: User, all_roles: set[str], death_triggers: bool):
    ...
    for vigilante, target in list(KILLS.items()):
        if target is player:
            await vigilante.send(messages["hunter_discard"])
            del KILLS[vigilante]
...
async def on_resolve_killer_tag(evt: Event, var: GameState, victim: User, killer: str):
    if killer == "@vigilante":
...
async def on_send_role(evt: Event, var: GameState):
    ...
    for vigilante in get_all_players(var, ("vigilante",)):
        ...
        random.shuffle(pl)
        pl.remove(vigilante)
        await vigilante.send(messages["vigilante_notify"])
        if var.next_phase == "night":
            await vigilante.send(messages["players_list"].format(pl))
